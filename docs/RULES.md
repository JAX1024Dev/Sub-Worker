...
中国域名/IP 直连；未匹配国内规则的流量由 `route.final = "🚀 节点选择"` 代理。每个热门服务使用独立 `selector`，可在支持 selector 的客户端界面选择全局组、具体节点或直连。选择由 sing-box `experimental.cache_file` 在本地保存，不存储在 Worker；`cache_id` 按订阅 ID 和平台派生。节点 tag 由名称和节点身份的稳定摘要组成，避免订阅顺序变化清除选择；节点改名或凭据变化会产生新 tag。
...
| 💷 Kraken/Krak                                   | UK 节点组   | `kraken.com`、`krak.app` 后缀         |

`uk` 组仅包含节点标签明确标识英国的节点。没有英国节点时默认 `block`，不得静默使用其他地区；标签不保证真实出口地理位置。Kraken/Krak 组允许用户改选全局组或具体节点。广告组允许用户关闭拦截，不能只用固定 block outbound。
...
4. Telegram、Apple、Microsoft（含 `microsoft@cn`）、GitHub、Google。服务规则必须先于中国通用规则，才能让 Apple 默认代理并让 Microsoft 在 App 内自由改选。
5. `geosite-cn → direct`。
...
服务类使用 DustinWin `ruleset_geodata` 的 `sing-box-ruleset` 提交 `264c234cdf8daf5cbe671dd981f9886991f0bf73`；Google/Telegram/Apple/Microsoft/GitHub 使用 MetaCubeX `meta-rules-dat` 提交 `d1363ad015e8bb0fdcb0eb08be518e161354c695`。国内域名/IP 沿用已验证的 SagerNet 固定提交。全部使用 pinned HTTPS `.srs` URL 和 `rules-via-proxy` HTTP client；不在 Worker 请求时访问浮动分支。

`update-service-rulesets.yml` 每日检查 DustinWin 最新提交及所需 SRS 文件，变动时更新 pin 和发布 bundle 并提出 PR。合并前运行 schema、单元测试、五平台 `sing-box check`，审查规则变化；production manifest 仍需单独晋级。MetaCubeX 和 SagerNet pin 手动评估更新。客户端的 `cache_file` 缓存远程规则集与 selector 状态；Worker 仍不缓存用户订阅。若远程规则集首次加载失败，客户端可能无法启动，应保留已验证的旧配置并在 staging 先验证。
